...
import { Search, CheckCircle, Clock, Star, MessageCircle } from 'lucide-react';
...
export function MyResolvedTickets() {
    ...
    const loadData = async () => {
      try {
        const [ticketsResult, usersData, categoriesData, prioritiesData, statusesData] = await Promise.all([
          apiService.getTickets({ userId: user?.id, page: currentPage, limit: pageSize, resolvedOnly: true }),
          apiService.getUsers(),
  ...
  // Filter for resolved/closed tickets only
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-gray-600 dark:text-gray-400">
          Tickets you have resolved or closed
        </p>
      </div>
        ...
        <Card>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {resolvedTickets.filter(t => t.status === 'Closed').length}
              </p>
            </div>
          </div>
        </Card>
        ...
        <Card>
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {resolvedTickets.filter(t => t.priority === 'urgent' || t.priority === 'high').length}
              </p>
            </div>
          </div>
        </Card>
